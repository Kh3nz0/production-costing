...
**The expanded live sweep ran, 22 September 2026.** The first run passed 59 checks and found one serious axe violation on Product Pricing: explanation paragraphs sat loose inside a definition-list group after its definitions. Moving each explanation into the relevant `<dd>` removed the violation. A focused rerun passed, then a full run against the rebuilt production app and live Supabase passed **60 of 60 available checks in 1.8 minutes**: all 52 available route variants, two redirects, concurrency, keyboard purchase/run/sale, sale focus and the phone layout. `/setup` remained the one explicit skip because a separate authenticated user without an organisation has not been supplied. The audit proves the rendered states reached by these fixtures; alternate data states and widths beyond the phone sale check are not implied by the count.
