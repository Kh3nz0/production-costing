...
The hosted Auth setting changed. A fresh read of `/auth/v1/settings` returned `disable_signup: true`, and the focused live `e2e/auth-config.spec.ts` check passed. The complete production browser suite then passed **93/93 in 2.6 minutes** with existing-account sign-in, all route audits, the concurrency check, keyboard flows and the 390px sale; that scripted sale saved in **0.44 seconds**. The emailed recovery link remains the final S1 check.

### S1 recovery-link correction, 22 September 2026

The first live emailed link, clicked about one minute after request in the same browser profile, landed on `/reset-password?error=link_expired`. This **does not prove the link expired**: the callback mapped every code-exchange error to that label. The reset request did set a PKCE verifier cookie in a browser probe. Two failure paths remained: without a flow id, a newer request can make an older email use the wrong verifier; and middleware called `getUser()` before the callback, so an invalid old session could sign out and clear the pending verifier. The exact error on the owner's first click was not recorded, so neither path is claimed as its proven cause.
