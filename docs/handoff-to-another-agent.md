...
| S1                 | Done. Sign-in, tenant isolation and private account creation are proven. Live Auth disables API signups. After the PKCE callback correction, a fresh emailed recovery link opened the new-password form; saving the password succeeded and redirected to the dashboard.                  |
...
**S1 recovery proof.** The first live click landed on `?error=link_expired` after one minute; that label was misleading because every exchange error used it. After the callback correction, a fresh live emailed link opened `Choose a new password` in the same browser profile. Saving a new password succeeded and redirected to the dashboard. Together with `disable_signup: true`, the signed-in checks and tenant-isolation tests, this closes S1.
